...
const Globes=(()=>{
 ...
 let revealStart=0,revealCountry=null,lastFrame=0,spin=0,revealDone=false,homeDrawn=false,zoom=1;
 const MAX_ZOOM=3.2;
 ...
 function draw(canvas,longitude,latitude,country=null,options={}){
  ...
  const size=canvas.width,mid=size/2,globeRadius=size*(options.radius||.472),r=globeRadius*(options.zoom||1),lon=longitude*radians,lat=latitude*radians;
  ...
  ctx.beginPath();ctx.arc(mid,mid,globeRadius,0,Math.PI*2);ctx.fillStyle=ocean;ctx.fill();ctx.save();ctx.clip();
  ...
  if(options.labels){
   const font=Math.max(15,size/48);ctx.font=`700 ${font}px ui-rounded, Arial, sans-serif`;ctx.textAlign='center';ctx.textBaseline='middle';
   const occupied=[];const candidates=[country,...COUNTRIES.filter(c=>c!==country)].filter(Boolean);
   for(const c of candidates){
    const [x,y,z]=project(unit([c.lon,c.lat]));if(z<.12||Math.hypot(x-mid,y-mid)>globeRadius-font*1.5)continue;
    const label=c.name,width=ctx.measureText(label).width+12,left=x-width/2,top=y+(c===country?font*2:0)-font;
    if(left<mid-globeRadius+4||left+width>mid+globeRadius-4||occupied.some(b=>left<b.right+6&&left+width>b.left-6&&top<b.bottom+5&&top+font>b.top-5))continue;
    occupied.push({left,right:left+width,top,bottom:top+font});
    ctx.lineWidth=font*.3;ctx.strokeStyle='#102a4f';ctx.strokeText(label,x,top+font,width);ctx.fillStyle=c===country?'#fff0a5':'#ffffff';ctx.fillText(label,x,top+font,width);
   }
  }
 ...
 function placeMarker(marker){if(!marker||marker.z<0)return;const canvas=document.getElementById('reveal-globe'),stage=document.querySelector('.map-stage'),box=canvas.getBoundingClientRect(),stageBox=stage.getBoundingClientRect();stage.style.setProperty('--landing-x',`${box.left-stageBox.left+marker.x/canvas.width*box.width}px`);stage.style.setProperty('--landing-y',`${box.top-stageBox.top+marker.y/canvas.height*box.height}px`)}
 function drawLanded(){if(!revealCountry||!revealDone)return;placeMarker(draw(document.getElementById('reveal-globe'),revealCountry.lon,revealCountry.lat*.82,revealCountry,{radius:.472,halo:true,zoom,labels:zoom>1.15}));document.getElementById('zoom-out').disabled=zoom<=1;document.getElementById('zoom-in').disabled=zoom>=MAX_ZOOM}
