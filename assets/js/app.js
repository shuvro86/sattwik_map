...
let index=0,clueCount=0,remaining=30000,last=performance.now(),resolved=false,paused=false,sound=false,discovered=new Set();
...
function speak(text,force=false){if(!('speechSynthesis' in window)||(!sound&&!force))return;AdventureAudio.duck(Math.max(4000,text.length*85));window.speechSynthesis.cancel();const voice=new SpeechSynthesisUtterance(text);voice.lang='en-US';voice.rate=.82;voice.pitch=1.12;window.speechSynthesis.speak(voice)}
...
function makeClues(c){
 ...
 const shore=c.landlocked?'Other countries surround it. It has no ocean coast.':'It has a coast. Imagine waving hello to the sea!';
 if(level==='easy')return [`Find this flag among the choices: ${c.flag}`,familiar?c.clues[0]:shore,region,first,prefix];
 if(level==='medium')return [familiar?c.clues[0]:shore,region,city,first,prefix];
...
function showAtlasCountry(i){
 atlasIndex=i;const c=COUNTRIES[i],detail=$('atlas-detail');detail.replaceChildren();
...
document.querySelectorAll('.atlas-open').forEach(b=>b.onclick=()=>{stopSpeech();$('atlas-dialog').showModal();$('country-search').value='';filterAtlas();updateTimer()});
